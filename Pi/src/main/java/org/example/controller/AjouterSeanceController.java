...
public class AjouterSeanceController {
    ...
    private boolean validerNom() {
        ...
        if (val.isEmpty()) { setErreur(nomField, errNom, "❌ Le nom est obligatoire !"); return false; }
        if (val.length() < 3) { setErreur(nomField, errNom, "❌ Minimum 3 caractères !"); return false; }
        if (val.length() > 50) { setErreur(nomField, errNom, "❌ Maximum 50 caractères !"); return false; }
        if (!val.matches("[a-zA-ZÀ-ÿ\\s]+")) { setErreur(nomField, errNom, "❌ Lettres uniquement !"); return false; }
    ...
    private boolean validerHeure(TextField field, Label label, boolean obligatoire) {
        ...
        if (val.isEmpty()) {
            if (obligatoire) { setErreur(field, label, "❌ L'heure est obligatoire !"); return false; }
    ...
    private boolean validerDuree() {
        ...
        if (val.isEmpty()) { setErreur(dureeField, errDuree, "❌ La durée est obligatoire !"); return false; }
        if (!val.matches("\\d+")) { setErreur(dureeField, errDuree, "❌ Nombre entier uniquement !"); return false; }
        int d = Integer.parseInt(val);
        if (d < 5) { setErreur(dureeField, errDuree, "❌ Minimum 5 minutes !"); return false; }
        if (d > 120) { setErreur(dureeField, errDuree, "❌ Maximum 120 minutes !"); return false; }
    ...
    private boolean validerDate() {
        ...
        if (val.isEmpty()) { setErreur(dateField, errDate, "❌ La date est obligatoire !"); return false; }
        if (!val.matches("\\d{4}-\\d{2}-\\d{2}")) { setErreur(dateField, errDate, "❌ Format invalide ! YYYY-MM-DD"); return false; }
        try {
            java.time.LocalDate date    = java.time.LocalDate.parse(val);
            java.time.LocalDate today   = java.time.LocalDate.now();
            java.time.LocalDate minDate = today.minusYears(1);
            if (date.isAfter(today))  { setErreur(dateField, errDate, "❌ La date ne peut pas être dans le futur !"); return false; }
            if (date.isBefore(minDate)) { setErreur(dateField, errDate, "❌ Date trop ancienne !"); return false; }
        } catch (Exception e) {
            setErreur(dateField, errDate, "❌ Date invalide !");
            return false;
        }
    ...
    private boolean validerUtilisateur() {
        ...
        if (val.isEmpty()) { setErreur(utilisateurIdField, errUtilisateur, "❌ L'ID utilisateur est obligatoire !"); return false; }
        if (!val.matches("\\d+")) { setErreur(utilisateurIdField, errUtilisateur, "❌ Nombre entier uniquement !"); return false; }
        if (Integer.parseInt(val) <= 0) { setErreur(utilisateurIdField, errUtilisateur, "❌ L'ID doit être positif !"); return false; }
    ...
    public void lancerAnalyseIA() {
        int userId = 1;
        String nomUser = "Utilisateur";

        String idSaisi = utilisateurIdField.getText().trim();
        if (!idSaisi.isEmpty() && idSaisi.matches("\\d+")) {
            userId = Integer.parseInt(idSaisi);
        }
        ...
        try {
            ...
            ctrl.setUtilisateur(userId, nomUser);

            // ✅ Remplace le contenu dans la même fenêtre
            btnMenu.getScene().setRoot(root);

        } catch (IOException e) {
            new Alert(Alert.AlertType.ERROR,
                    "❌ Impossible d'ouvrir l'Analyse IA :\n" + e.getMessage()
            ).showAndWait();
            e.printStackTrace();
        }
    }
