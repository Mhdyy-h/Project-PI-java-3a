    ...
    private void setupForUserRole() {
        if (currentUser != null) {
            if (currentUser.isPatient()) {
                // Hide patient section for patients - they can only book for themselves
                ...
                System.out.println("🔍 Patient mode: Auto-selected current user as patient: " + currentUser.getNomComplet());
            }
            // Admin and specialists see all fields
        ...
        try {
            ...
            if (mode == Mode.ADD) {
                ...
                try {
                    if (RendezVousDAO.createRendezVous(rendezVous)) {
                        ...
                        try {
                            ...
                            successMessage += "�‍⚕️ Spécialiste: " + specialisteComboBox.getValue().getNomDocteur() + "\n";
                            successMessage += "👤 Patient: " + patientComboBox.getValue().getNomComplet() + "\n\n";
                        ...
                        // Notify parent controller about new appointment
                        if (dialogStage != null && dialogStage.getOwner() != null) {
                            javafx.scene.Scene ownerScene = dialogStage.getOwner().getScene();
                            if (ownerScene != null && ownerScene.getRoot() != null) {
                                // Try to find the parent controller
                                Object controller = ownerScene.getRoot().getUserData();
                                if (controller instanceof org.example.controller.RendezVousController) {
                                    ((org.example.controller.RendezVousController) controller).addAppointmentNotification(rendezVous, "created");
                                }
                            }
                        }
                ...
                if (RendezVousDAO.updateRendezVous(rendezVous)) {
                    ...
                    // Notify parent controller about modified appointment
                    if (dialogStage != null && dialogStage.getOwner() != null) {
                        javafx.scene.Scene ownerScene = dialogStage.getOwner().getScene();
                        if (ownerScene != null && ownerScene.getRoot() != null) {
                            // Try to find the parent controller
                            Object controller = ownerScene.getRoot().getUserData();
                            if (controller instanceof org.example.controller.RendezVousController) {
                                ((org.example.controller.RendezVousController) controller).addAppointmentNotification(rendezVous, "modified");
                            }
                        }
                    }
    ...
    private boolean validateInput() {
        ...
        if (patientComboBox.getValue() == null) {
            if (currentUser != null && currentUser.isPatient()) {
                // For patient users, auto-select current user if not already selected
                patientComboBox.setValue(currentUser);
                System.out.println("🔍 Auto-selected current patient for validation: " + currentUser.getNomComplet());
            } else {
