...
public class RendezVousDAO {
    ...
    public static List<RendezVous> getAllRendezVous() {
        ...
        String query = "SELECT r.*, " +
                    "u1.nom_complet as patient_nom, " +
                    "s.nom_docteur as specialiste_nom " +
                    "FROM rendez_vous r " +
                    "LEFT JOIN utilisateur u1 ON r.patient_id = u1.id " +
                    "LEFT JOIN specialiste s ON r.specialiste_id = s.utilisateur_id " +
    ...
    public static RendezVous getRendezVousById(Integer id) {
        String sql = "SELECT r.*, u1.nom_complet as patient_nom, s.nom_docteur as specialiste_nom " +
                    "FROM rendez_vous r " +
                    "LEFT JOIN utilisateur u1 ON r.patient_id = u1.id " +
                    "LEFT JOIN specialiste s ON r.specialiste_id = s.utilisateur_id " +
                     "WHERE r.id = ?";
    ...
    // READ - Get rendez-vous by specialiste ID
    public static List<RendezVous> getRendezVousBySpecialiste(int specialisteId) {
        List<RendezVous> rendezVousList = new ArrayList<>();
        String query = "SELECT r.*, " +
                    "u1.nom_complet as patient_nom, " +
                    "s.nom_docteur as specialiste_nom " +
        ...
        System.out.println("🔍 DEBUG: Getting RDVs for specialiste ID: " + specialisteId);
        System.out.println("🔍 DEBUG: Executing SQL: " + query);
        ...
        try (Connection conn = DatabaseConnection.getFreshConnection();
            ...
            System.err.println("Error getting rendez-vous by specialiste: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public static List<RendezVous> searchRendezVous(String keyword, String statut, Integer specialisteId) {
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT rv.*, u1.nom_complet as patient_nom, s.nom_docteur as specialiste_nom " +
            "FROM rendez_vous rv " +
            "LEFT JOIN utilisateur u1 ON rv.patient_id = u1.id " +
            "LEFT JOIN specialiste s ON rv.specialiste_id = s.utilisateur_id " +
            "WHERE 1=1"
        );
    ...
    private static RendezVous mapResultSetToRendezVous(ResultSet rs) throws SQLException {
        ...
        try {
            String patientName = rs.getString("patient_nom");
            System.out.println("🔍 DEBUG: Raw patient_name from DB: " + patientName);
            if (patientName != null && !patientName.trim().isEmpty()) {
                rv.setPatientNom(patientName);
            } else {
                rv.setPatientNom("Patient");
            }
        } catch (SQLException e) {
            System.out.println("🔍 DEBUG: patient_nom column not found, using default");
            rv.setPatientNom("Patient");
        }
    ...
    public static List<RendezVous> getRendezVousByStatut(String statut) {
        ...
        String sql = "SELECT * FROM rendez_vous WHERE statut = ?";
